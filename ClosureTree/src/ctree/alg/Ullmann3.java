...
/**
 * Simplified version of Ullmann2.
 * Edge are labels starting from integer 1
...
public class Ullmann3 {
  ...
  /**
   *
   * @param _adj1 adjacency matrix of graph 1, [i][j]==0 if no edge, >0 o/w
  ...
  private static boolean checkMapping(int depth, int v) {
    for (int j = 0; j < n1; j++) {
      ...
      if (adj1[depth][j] !=0) {
        if (map[j] != -1) {
          if (adj1[depth][j]!=adj2[v][map[j]]) {
            return false;
          }
        } else {
          boolean flag = false;
          for (int l = 0; l < n2; l++) {
            if (M[j][l] == 1 && rmap[l]==-1&& adj2[v][l]==adj1[depth][j]) {
              flag = true;
              break;
            }
          }

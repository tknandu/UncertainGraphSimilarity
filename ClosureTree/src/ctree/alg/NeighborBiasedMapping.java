...
public class NeighborBiasedMapping {
     ...
     * @return vertex map from G1 to G2
     */
    public static int[] mapGraphsAndReturnProb(LGraph g1, LGraph g2, int n1, int n2, double[][] W, int[][] alist1,
                                  int[][] alist2, int[][] bilist,double bonus, double userProbThreshold, RankerEntry entry) {
          ...
          for(int v1:alist2[e.v]){ // neighbours of v
          	if(rmap[v1]>=0){ // if neighbour of v is already mapped
        			for(int neigh : alist1[e.u]){
              		...
              		if(e.v<v1){
              			//System.out.println(e.v+","+v1+" "+g2.probMap.get(e.v+","+v1));
              			probOfMappedG2 = probOfMappedG2 * g2.probMap.get(e.v+","+v1);
              		}
              		else {
              			//System.out.println(v1+","+e.v+" "+g2.probMap.get(v1+","+e.v));
              			probOfMappedG2 = probOfMappedG2 * g2.probMap.get(v1+","+e.v);
              		}          					
        					break;
        				}
        			}
